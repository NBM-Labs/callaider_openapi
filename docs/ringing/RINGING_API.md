...
> **Важливо:** Конфігурації **постаналізу**, **вебхуків** та **воркфлоу після дзвінка** створюються та налаштовуються в **особистому кабінеті** Callaider. Через API ви можете отримати їх список та використати `id` при створенні кампанії. Це ж стосується **AI-асистентів** — вони створюються в кабінеті, а через API можна отримати список доступних асистентів (`GET /assistants`) та використати `id` при створенні кампанії.
...
> Створюються та редагуються в **особистому кабінеті**. Через API доступний безпечний довідник без внутрішньої конфігурації воркфлоу.
