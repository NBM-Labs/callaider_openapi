...
3. [API Reference](#api-reference)
   ...
   - [Воркфлоу після дзвінка](#воркфлоу-після-дзвінка)
   - [Воркфлоу після завершення кампанії](#воркфлоу-після-завершення-кампанії)
...
│                                  GET /webhook-configs               │
│                                  GET /post-call-workflows           │
│                                  GET /post-campaign-workflows       │
...
> **Важливо:** Конфігурації **постаналізу**, **вебхуків**, **воркфлоу після дзвінка** та **воркфлоу після завершення кампанії** створюються та налаштовуються в **особистому кабінеті** Callaider. Через API ви можете отримати їх список та використати `id` при створенні кампанії. Це ж стосується **AI-асистентів** — вони створюються в кабінеті, а через API можна отримати список доступних асистентів (`GET /assistants`) та використати `id` при створенні кампанії.
...
#### Отримати воркфлоу після дзвінка

```http
GET /v1/ringing/post-call-workflows
Authorization: Bearer cld_ваш_ключ
```
...
[
  {
    "id": 7,
    "name": "Передати результат у CRM",
    "description": "Створює угоду після завершення дзвінка",
    "isActive": true
  }
]
```

> Використовуйте `id` як `postCallWorkflowId` при створенні або оновленні кампанії. Endpoint повертає лише воркфлоу з контекстом `ringing.post_call`.

#### Отримати воркфлоу після завершення кампанії

```http
GET /v1/ringing/post-campaign-workflows
Authorization: Bearer cld_ваш_ключ
```
...
[
  {
    "id": 9,
    "name": "Передати результати кампанії",
    "description": "Відправляє один payload після завершення запуску",
    "isActive": true
  }
]
```

> Використовуйте `id` як `postCampaignWorkflowId` при створенні або оновленні вихідної кампанії. Endpoint повертає лише воркфлоу з контекстом `ringing.post_campaign`.
...

{
  ...
  "postCallWorkflowId": 7,
  "postCampaignWorkflowId": 9,
  "webhookConfigId": 1,
  "sipAccountIds": [10, 11]
}
```
...
| `postCallWorkflowId` | `number \| null` | Ні | ID воркфлоу після дзвінка (отримати через `GET /post-call-workflows`). `null` вимикає воркфлоу |
| `postCampaignWorkflowId` | `number \| null` | Ні | ID воркфлоу з `GET /post-campaign-workflows`, що виконується один раз після завершення окремого запуску кампанії |
...
|---|---|---|---|
| `maxCallTryCount` | `number` | `3` | Максимальна кількість спроб дозвону на абонента (1–5). Зберігається при першому запуску, ігнорується при повторних |
...
- Кожен початковий або повторний запуск має окремий `campaignRunId`; post-campaign воркфлоу отримує тільки дзвінки цього запуску одним масивом
...
### Воркфлоу після дзвінка

> Створюються та редагуються в **особистому кабінеті**.

| Метод | Шлях | Опис |
|---|---|---|
| `GET` | `/post-call-workflows` | Список воркфлоу компанії з контекстом `ringing.post_call` |

Використовуйте `id` з відповіді як `postCallWorkflowId` при створенні або оновленні кампанії. Переданий ID перевіряється на належність компанії та правильний контекст.

**Поля довідника:**

| Поле | Тип | Опис |
|---|---|---|
| `id` | `number` | Ідентифікатор воркфлоу |
| `name` | `string` | Назва воркфлоу |
| `description` | `string \| null` | Опис воркфлоу |
| `isActive` | `boolean` | Чи активний воркфлоу |

---

### Воркфлоу після завершення кампанії

> Створюються та редагуються в **особистому кабінеті**.

Воркфлоу виконується один раз після того, як усі дзвінки та постаналіз окремого запуску перейшли у фінальний стан. Початковий запуск і кожен re-launch обробляються окремо.

| Метод | Шлях | Опис |
|---|---|---|
| `GET` | `/post-campaign-workflows` | Список воркфлоу компанії з контекстом `ringing.post_campaign` |

Використовуйте `id` з відповіді як `postCampaignWorkflowId` при створенні або оновленні вихідної кампанії.
